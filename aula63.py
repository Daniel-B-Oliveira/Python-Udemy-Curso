...
while True:
    ...
    try:
        ...
        cpf_tratado_str = re.sub(r'[^0-9]', '', cpf)
        cpf_tratado = int(cpf_tratado_str)

    cpf_repetido = str(cpf_tratado)[0] * int(len(str(cpf_tratado)))  # cpf tratada % 11111111111 == 0

    if len(str(cpf_tratado)) != 11 or cpf_tratado == cpf_repetido:
        print('Números incompátiveis.')
        continue
